...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Make all face exclusion records point to the root of the campaign
        fallback chain, rather than to the campaign to which the user had fallen
        back before sharing.

        """
        fallbacks = orm.CampaignProperties.objects.exclude(campaign=models.F('root_campaign'))
        for (root_campaign, props_group) in groupby(
            fallbacks.order_by('root_campaign').iterator(),
            attrgetter('root_campaign')
        ):
            campaign_ids = [props.campaign_id for props in props_group]
            fallback_exclusions = orm.FaceExclusion.objects.filter(campaign__in=campaign_ids)

            # Rather than just set campaign to root, we have to DELETE those
            # with otherwise identical signatures, as then they would be truly
            # duplicative, and run afoul of the table's uniqueness constraint.
            unique_keys = ('fbid', 'friend_fbid', 'content')
            for (unique_values, exclusions_group) in groupby(
                fallback_exclusions.order_by(*unique_keys).iterator(),
                attrgetter(*unique_keys)
            ):
                unique_map = dict(zip(unique_keys, unique_values))
                edge_exclusions = orm.FaceExclusion.objects.filter(**unique_map)
                if edge_exclusions.filter(campaign=root_campaign).exists():
                    # We already have what we need
                    dupes = fallback_exclusions & edge_exclusions
                    dupes.delete()
                else:
                    first_exclusion = next(exclusions_group)
                    remainder = [exclusion.campaign_id for exclusion in exclusions_group]
                    if remainder:
                        edge_exclusions.filter(campaign__in=remainder).delete()
                    first_exclusion.campaign = root_campaign
                    first_exclusion.save()
