...
INACTIVE_URLS = (
    ...
    ('ameralums', 'http://www.americorpsalums.org/'),
    ('demo', 'http://www.edgeflip.com/'),
    ('edf', 'http://www.edf.org/'),
    ('edgeflip', 'http://www.edgeflip.com/'),
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Set URLs to which to redirect visitors for existing clients' campaigns."""
        for (codename, url) in INACTIVE_URLS:
            updated = orm.Client.objects.filter(codename=codename).update(campaign_inactive_url=url)
            self.puts("{warning}Set campaign inactive URL for {} clients ({}): {}",
                      updated, codename, url, warning=("[WARNING] " if not updated else ""))

    def backwards(self, orm):
        """Clear inactive campaign URLs."""
        orm.Client.objects.update(campaign_inactive_url="")
        self.puts("Cleared all clients' inactive campaign URLs")
