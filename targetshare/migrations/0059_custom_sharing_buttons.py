...
DEFAULT_TEXT = 'Show Your Support'

CUSTOM_TEXT = (
    ('hampton', "Join The Movement"),
    ('terrymcauliffe', "Post Your Message"),
)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for (codename, text) in CUSTOM_TEXT:
            client_fbattrs = orm.FBObjectAttribute.objects.filter(
                fb_object__client__codename=codename,
            )
            count = client_fbattrs.update(sharing_button=text)
            self.puts('Set "{}" for {} {} FBObjects'.format(text, count, codename))

    def backwards(self, orm):
        count = orm.FBObjectAttribute.objects.exclude(
            sharing_button=DEFAULT_TEXT,
        ).update(sharing_button=DEFAULT_TEXT)
        self.puts('Reverted {} FBObjects to "{}"'.format(count, DEFAULT_TEXT))
