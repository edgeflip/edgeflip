...
CLIENT_DATA = (
    ('virginia', 'sharing', 'terrymcauliffe'),
    ('nypr', 'betterpassword', 'wnyc'),
    ('global_giving', 'givebetterpassword', 'global-giving')
)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for name, password, codename in CLIENT_DATA:
            try:
                client = orm['targetshare.Client'].objects.get(codename=codename)
            except orm['targetshare.Client'].DoesNotExist:
                # Client must not exist in this env
    ...
    def backwards(self, orm):
        ...
        for name, password, codename in CLIENT_DATA:
            ...
            orm['auth.User'].objects.filter(username=name).delete()
            orm['auth.Group'].objects.filter(name=name).delete()
            client.auth_groups.clear()
    ...
    models = {
        ...
        'targetshare.client': {
            ...
            'auth_groups': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'to': u"orm['auth.Group']", 'null': 'True', 'blank': 'True'}),
