...
if settings.ENV == 'staging':
    FB_APPS = [
        (417233888375210, u'social-organizations'),
    ]
else:
    FB_APPS = [
        (471727162864364, u'sharing-social-good'),
        (111399285589490, u'org-for-action'),
        (978775852135782, u'share-for-good'),
    ]
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Populate FB apps and assign to Clients."""
        # Populate secrets (eagerly to error out before database operations)
        secrets = {}
        for (appid, appname) in FB_APPS:
            try:
                secret = settings.FACEBOOK.secrets[str(appid)]
            except KeyError:
                if settings.ENV in ('development', 'staging'):
                    self.puts("[WARNING] no secret found for '{}' ({})", appname, appid)
                else:
                    raise
            else:
                secrets[appid] = secret

        # Insert apps, assign permissions, and assign apps to clients
        for (appid, appname) in FB_APPS:
            clients = orm.Client.objects.filter(fb_app_id=appid)

            try:
                secret = secrets[appid]
            except KeyError:
                codenames = clients.values_list('codename', flat=True)
                if codenames:
                    self.puts("[WARNING] cannot assign '{}' ({}) to {} clients "
                              "due to missing secret ({})",
                              appname, appid, len(codenames), codenames)
                continue

            app = orm.FBApp.objects.create(
                appid=appid,
                name=appname,
                secret=secret,
                api=1,
            ...
            try:
                ...
                self.puts("[WARNING] no clients in {} environment use app '{}' ({}): "
                          "assigning no permissions", settings.ENV, appname, appid)
            else:
                app.permissions = client.fb_app_permissions.all()
                clients.update(fb_app=app)

    def backwards(self, orm):
        """Clear FB apps"""
        orm.Client.objects.update(fb_app=None)
        orm.FBApp.objects.all().delete()
