...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Assign default page styles for the button page"""
        button_page = orm.Page.objects.get(code=BUTTON)
        page_styles = button_page.pagestyles.get(
            starred=True,
            client=None,
        )
        for campaign in orm.Campaign.objects.exclude(
            campaignpagestylesets__page_style_set__page_styles__page=button_page
        ):
            style_set = page_styles.pagestylesets.create()
            campaign.campaignpagestylesets.create(
                page_style_set=style_set,
                rand_cdf=1,
            )
            self.puts("Attached default page style [{}] to {} campaign [{}]",
                      page_styles.pk, campaign.client.codename, campaign.pk)

    def backwards(self, orm):
        """Remove page styles for the button page"""
        orm.PageStyleSet.objects.filter(
            page_styles__page__code=BUTTON
        ).delete()
