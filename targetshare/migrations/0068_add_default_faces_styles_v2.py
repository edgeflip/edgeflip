...
css_url_context = {
    ...
    'env_initial': ('p' if settings.ENV == 'production' else 's'),
}

css_url_template = '{base_url}{env_initial}/c/{{}}.css'.format(**css_url_context)
...
if settings.ENV == 'development':
    OLD_DEFAULT_CSS_URL = cssurl('edgeflip-base-1')
    NEW_DEFAULT_CSS_URL = cssurl('edgeflip-base-2')
else:
    OLD_DEFAULT_CSS_URL = cssurl('edgeflip-base-1.min')
    NEW_DEFAULT_CSS_URL = cssurl('edgeflip-base-2.min.gz')
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Add PageStyle for new frame faces default CSS and make it the default."""
        ...
        page.pagestyles.create(
            name="Edgeflip default faces style v2",
            description="Default faces stylesheet provided by Edgeflip (version 2)",
            client=None,
            starred=True,
            url=NEW_DEFAULT_CSS_URL,
        )
        self.puts("Starred {} default page style", NEW_DEFAULT_CSS_URL)

    def backwards(self, orm):
        """Remove new PageStyle and restore old default."""
        page = orm.Page.objects.get(code=FRAME_FACES)
        page.pagestyles.filter(starred=True, client=None, url=NEW_DEFAULT_CSS_URL).delete()
        page.pagestyles.filter(client=None, url=OLD_DEFAULT_CSS_URL).update(starred=True)
