...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Add campaign & content IDs to "heartbeat" events."""
        visits = orm.Visit.objects.filter(
            events__event_type='heartbeat',
            events__campaign_id=None,
        ).distinct()

        event_count = 0
        visit_count = 0
        for (visit_count, visit) in enumerate(visits, 1):
            # In pre-history we repeated some session_start events ;(
            # but that's before the time period of this heartbeat issue:
            session_start = visit.events.get(event_type='session_start')
            visit.events.filter(event_type='heartbeat').update(
                campaign=session_start.campaign_id,
                client_content=session_start.client_content_id,
            )
            if session_start.campaign_id and session_start.client_content_id:
                event_count += 1
            else:
                # Again, in pre-history, plenty of session_start events were
                # missing campaign_id, but nowadays we should be fine.
                # Nonetheless, report issues:
                self.puts("Session start {}: campaign {} content {}".format(
                    session_start.pk,
                    session_start.campaign_id,
                    session_start.client_content_id,
                ))

        self.puts("Set campaign and client-content IDs on heartbeat events of "
                  "{} visits (out of {})".format(event_count, visit_count))
