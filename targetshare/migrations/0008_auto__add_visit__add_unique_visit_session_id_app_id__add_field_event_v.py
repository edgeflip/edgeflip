...
class Migration(SchemaMigration):
    """Add Visit model and Event.visit field.

    Leaves Event.visit nullable, until Visits can be populated.

    """
    def forwards(self, orm):
        # Adding model 'Visit'
        db.create_table('visits', (
            ('visit_id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
        ...
        db.send_create_signal('targetshare', ['Visit'])

        # Adding unique constraint on 'Visit', fields ['session_id', 'app_id']
        db.create_unique('visits', ['session_id', 'appid'])

        # Adding field 'Event.visit'
        # (Allow null until visits are populated)
        db.add_column('events', 'visit',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['targetshare.Visit'], null=True),
                      keep_default=False)
    ...
    def backwards(self, orm):
        ...
        # Deleting field 'Event.visit'
        db.delete_column('events', 'visit_id')
