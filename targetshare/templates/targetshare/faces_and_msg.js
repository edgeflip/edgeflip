...
function selectFriend(fbid) {
    ...
    if (isRecip(fbid)) {  // if the friend is already in the recips list, do nothing
        ...
        if (edgeflip.Faces.debug) {
            edgeflip.Event.record('selected_friend', {friends: [fbid]});
        }
        return true;
    }
}
...
function unselectFriend(fbid) {
    ...
    if (isRecip(fbid)) {
        ...
        reformatRecipsList();
        if (edgeflip.Faces.debug) {
            edgeflip.Event.record('unselected_friend', {friends: [fbid]});
        }
        return true;
    } else {
        return false;
...
function useSuggested(msgs) {
    edgeflip.Event.record('suggest_message_click');
...
function selectAll(skipRecord) {
    if (!skipRecord) {
        edgeflip.Event.record('select_all_click');
    }
    ...
    FB.api(
        ...
        function(response) {
            if (!response || response.error) {
                ...
                edgeflip.Event.record('share_fail', {
...
function doShare() {
    ...
    if (getRecipFbids().length == 0) {
        if (confirm("You haven't chosen any friends to share with.\n\nClick OK to share with all suggested friends or CANCEL to return to the page.")) {
            ...
            if (edgeflip.Faces.debug) {
                edgeflip.Event.record('empty_share');
            }
            return;
        }
    }
    edgeflip.Event.record('share_click');
