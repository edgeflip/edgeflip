...
def get_new_uuid(orm):
    "Returns uuid that isn't being used."
    while True:
        uuid = get_random_string(40, string.ascii_uppercase + string.digits)
        if not orm.Visitor.objects.filter(uuid=uuid).exists():
            return uuid
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Populate Visitors for existing Visits."""
        for visit in orm.Visit.objects.filter(fbid=None):
        ...
        for fbid in orm.Visit.objects.exclude(fbid=None).values_list('fbid', flat=True).distinct():
            ...
            orm.Visit.objects.filter(fbid=fbid).update(visitor=visitor)

    def backwards(self, orm):
        """Clear Visitors."""
        orm.Visit.objects.update(visitor=None)
        orm.Visitor.objects.all().delete()
