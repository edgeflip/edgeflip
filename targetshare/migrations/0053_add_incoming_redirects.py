...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Add missing "incoming_redirect" events."""
        visits = orm.Visit.objects.filter(
            events__event_type='auth_fail',
            events__content='oauth',
        ).exclude(
        ...
        for (visit_count, visit) in enumerate(visits, 1):
            auth_fail = visit.events.filter(event_type='auth_fail').order_by('event_datetime')[0]
            (_event, created) = visit.events.get_or_create(
                event_type='incoming_redirect',
                defaults={
                    'campaign_id': auth_fail.campaign_id,
                    'client_content_id': auth_fail.client_content_id,
                },
            )
            assert created

        self.puts('Added event "incoming_redirect" to {} visits'.format(visit_count))
