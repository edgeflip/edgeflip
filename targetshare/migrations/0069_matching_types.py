...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Insert Edgeflip voter matching service into filter feature type table and
        adjust surrounding feature types' names and sort ordering.

        """
        orm.FilterFeatureType.objects.filter(code='matching').update(
            code='civis_voter',
            name='Voter Score (Civis)',
            sort_order=6,
        )
        orm.FilterFeatureType.objects.filter(code='topics').update(sort_order=7)
        orm.FilterFeatureType.objects.get_or_create(
            code='ef_voter',
            defaults={
                'name': "Voter Score (Edgeflip)",
                'px_rank': 3,
                'sort_order': 5,
            },
        )
    ...
    def backwards(self, orm):
        orm.FilterFeatureType.objects.filter(code='civis_voter').update(
            code='matching',
            name='Matching',
            sort_order=5,
        )
        orm.FilterFeatureType.objects.filter(code='topics').update(sort_order=6)
        orm.FilterFeatureType.objects.filter(code='ef_voter').delete()
