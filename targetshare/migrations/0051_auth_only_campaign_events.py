...
AUTH_ONLY_CAMPAIGN_ID = 154
AUTH_ONLY_CONTENT_ID = 73
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Add "authorized" event to visits of our first non-faces (auth-only) campaign."""
        visits = orm.Visit.objects.filter(
            events__campaign_id=AUTH_ONLY_CAMPAIGN_ID,
            events__event_type='incoming_redirect',
            events__content__contains="code=",
        ).distinct()

        visit_count = 0
        event_count = 0
        for (visit_count, visit) in enumerate(visits, 1):
            (_event, created) = visit.events.get_or_create(
                event_type='authorized',
                defaults={
                    'campaign_id': AUTH_ONLY_CAMPAIGN_ID,
                    'client_content_id': AUTH_ONLY_CONTENT_ID,
                    'content': 'oauth',
                },
            )
            event_count += int(created)

        self.puts('Added event "authorized" to {} non-faces visits (out of {})'.format(event_count, visit_count))

    def backwards(self, orm):
        """Remove "authorized" event from visits limited to the non-faces campaign."""
        events = orm.Event.objects.filter(event_type='authorized', campaign_id=AUTH_ONLY_CAMPAIGN_ID)
        event_count = events.count()
        events.delete()
        self.puts('Deleted {} "authorized" events from non-faces visits'.format(event_count))
