...
CAMPAIGN_EXPR = re.compile(r'rs=ef([0-9]+)')
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Add campaign to "outgoing_redirect" events."""
        events = orm.Event.objects.filter(
            event_type='outgoing_redirect',
            campaign_id=None,
            content__regex=CAMPAIGN_EXPR.pattern,
        )

        count = 0
        for (count, event) in enumerate(events.iterator(), 1):
            match = CAMPAIGN_EXPR.search(event.content)
            event.campaign_id = match.group(1)
            event.save(update_fields=['campaign_id'])

        self.puts('Set campaign on {} "outgoing_redirect" events'.format(count))

    def backwards(self, orm):
        """Clear campaign from all "outgoing_redirect" events."""
        count = (
            orm.Event.objects
            .filter(event_type='outgoing_redirect')
            .exclude(campaign_id=None)
        ).update(campaign=None)
        self.puts('Cleared campaign from {} "outgoing_redirect" events'.format(count))
