...
# Campaigns created outside the wizard, whose content we can't easily guess from the names
LEGACY_LINKS = {
    # Mayors
    3: 2, # 'MAIG NV Campaign': 'NV Gun Reform Call Sandoval'
    168: 80, # 'hey 1': 'Mayors Against Illegal Guns hey'

    # McAuliffe
    6: 3, # '20130621 New McAuliffe Auth ver. A': 'McAuliffe VA Auth Campaign Dummy'
    7: 3,
    10: 3,
    16: 5,
    68: 27,
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Populate CampaignProperties.client_content."""
        # Fallback campaigns' content should be whatever's already configured
        # in CampaignProperties.fallback_content, defaulting to the root
        # campaign's content; it's determining the root content that's tough.
        root_properties = orm.CampaignProperties.objects.filter(root_campaign=models.F('campaign'))
        for (root_campaign, properties_group) in groupby(root_properties.order_by('campaign'), attrgetter('campaign')):
            # Ensure campaign doesn't have multiple sets of properties
            try:
                (properties,) = properties_group
            except ValueError:
                self.puts("[WARNING] multiple sets of properties for campaign {} [SKIP]", root_campaign.pk)
                continue

            # Check for client-content following wizard naming convention
            client = root_campaign.client
            contents = client.clientcontent.filter(name='{} {}'.format(client.name, root_campaign.name[:-2]))
            content_ids = contents.values_list('pk', flat=True)
            if len(content_ids) > 1:
                self.puts("[WARNING] multiple possible contents for campaign {} [SKIP]", root_campaign.pk)
                continue
            try:
                (client_content_id,) = content_ids
            except ValueError:
                # Look up client-content in hardcoded table
                try:
                    client_content_id = LEGACY_LINKS[root_campaign.pk]
                except KeyError:
                    self.puts("[WARNING] failed to find appropriate client content for campaign {} [SKIP]", root_campaign.pk)
                    continue

            # Store link
            properties.client_content_id = client_content_id
            properties.save()

            # Store fallback campaign links
            fallback_campaign = properties.fallback_campaign
            fallback_content_id = properties.fallback_content_id or client_content_id
            while fallback_campaign:
                # Store chain's fallback content as fallback campaign's default content
                fallback_properties = fallback_campaign.campaignproperties.get()
                fallback_properties.client_content_id = fallback_content_id
                fallback_properties.save()

                # Check for next fallback
                fallback_campaign = fallback_properties.fallback_campaign
                fallback_content_id = fallback_properties.fallback_content_id or fallback_content_id

        # Report remainder (skipped or missed)
        remainder = orm.CampaignProperties.objects.filter(client_content=None).count()
        if remainder:
            self.puts("[CRITICAL] properties-sets not updated ({})", remainder)
            self.puts("[CRITICAL] (you should try manually assigning these in another thread)")
            raw_input("Continue? ")

    def backwards(self, orm):
        """Revert CampaignProperties.client_content to None."""
        orm.CampaignProperties.objects.update(client_content=None)
