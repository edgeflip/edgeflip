...
class TestUtils(unittest.TestCase):
    ...
    def test_decode_date_valid(self):
        bday = facebook.utils.decode_date('02/01/2000')
        self.assertEqual(
            bday,
    ...
    def test_decode_date_invalid_leap_year(self):
        bday = facebook.utils.decode_date('02/29/1905')
        self.assertEqual(bday, None)

    def test_decode_date_valid_leap_year(self):
        bday = facebook.utils.decode_date('02/29/2004')
        self.assertEqual(
            bday,
...
class FakeResponse(object):
    ...
    @classmethod
    def handle_request(cls, method, url, params, *args, **kws):
        if method != 'GET':
        ...
        for (suffix, response) in (
            ...
            ('photos/uploaded', UPLOADED_PHOTOS_RESPONSE),
            ('videos', VIDEO_RESPONSE),
            ('videos/uploaded', UPLOADED_VIDEO_RESPONSE),
        ):
...
@patch('requests.sessions.Session.request', FakeResponse.handle_request)
class TestClient2(unittest.TestCase):

    user = object()
    token = 't0keN'

    def assertNumInteractionsOfType(self, post, type_, num):
        self.assertEqual(
            sum(1 for interaction in post.interactions if interaction.type == type_),
            num
        )

    def read_stream(self, *permissions):
        stream = facebook.client2.Stream.read(self.user, self.token, permissions)
        stream.sort(key=lambda post: post.post_id)
        return stream

    def test_read_user_status(self):
        (link_post, status_post) = self.read_stream('user_status')

        self.assertEqual(len(link_post.interactions), 7)
        self.assertNumInteractionsOfType(link_post, 'link_likes', 2)
        self.assertNumInteractionsOfType(link_post, 'link_comms', 5)

        self.assertEqual(len(status_post.interactions), 12)
        self.assertNumInteractionsOfType(status_post, 'place_tags', 2)
        self.assertNumInteractionsOfType(status_post, 'stat_likes', 3)
        self.assertNumInteractionsOfType(status_post, 'stat_tags', 2)
        self.assertNumInteractionsOfType(status_post, 'stat_comms', 5)

    def test_read_user_photos(self):
        (uploads_post, photos_post) = self.read_stream('user_photos')

        self.assertEqual(len(uploads_post.interactions), 9)
        self.assertNumInteractionsOfType(uploads_post, 'photo_upload_likes', 4)
        self.assertNumInteractionsOfType(uploads_post, 'photo_upload_comms', 5)

        self.assertEqual(len(photos_post.interactions), 7)
        self.assertNumInteractionsOfType(photos_post, 'photo_comms', 5)
        self.assertNumInteractionsOfType(photos_post, 'photos_target', 1)
        self.assertNumInteractionsOfType(photos_post, 'photo_tags', 1)

    def test_read_user_videos(self):
        (uploads_post, videos_post) = self.read_stream('user_videos')

        self.assertEqual(len(videos_post.interactions), 14)
        self.assertNumInteractionsOfType(videos_post, 'video_tags', 12)
        self.assertNumInteractionsOfType(videos_post, 'video_comms', 1)
        self.assertNumInteractionsOfType(videos_post, 'videos_target', 1)

        self.assertEqual(len(uploads_post.interactions), 9)
        self.assertNumInteractionsOfType(uploads_post, 'video_upload_likes', 4)
        self.assertNumInteractionsOfType(uploads_post, 'video_upload_comms', 5)
