define(
    ...
    var facesAndMsg = function( data ) {
        ...
        $('#' + this.RECIPS_LIST_CONTAINER).on(
            'click', '.message_x', this.unselectFriendClicked.bind(this) );
        
        $('#' + this.RECIPS_LIST_CONTAINER).on(
            'click', '.added_x', this.removeFriendClicked.bind(this) );

        $('#button_sugg_msg').on( 'click', this.useSuggested.bind(this) );
        $('#button_select_all').on( 'click', this.selectAll.bind(this) );
        $('.friend_box').on( 'click', function() { self.selectAll( $(this).data('id') ); } );


        if ( this.faceFriends.length === 0) {
            ...
            faces.outgoingRedirect(faces.errorURL);
        }

        this.init();
    }

    $.extend( facesAndMsg.prototype, {

        unselectFriendClicked: function( e ) {
            this.unselectFriend( $(e.currentTarget).data('id') );
        },
        
        removeFriendClicked: function( e ) {
            this.unselectFriend( $(e.currentTarget).data('id') );
        },

        getRecipElts: function() {
            return $('#message_form_editable .recipient');
        },
        
        getRecipFbids: function() {
            var fbids = [];
            var recipElts = this.getRecipElts();
        ...
        isRecip: function(fbid) {
            return (this.getRecipFbids().indexOf(fbid) > -1);
        },

        htmlRecip: function(fbid) {
            var html = "<span class='recipient message_friend_name' id='recipient-"+fbid+"' contentEditable='False'>";
            html += this.friendFromFbid[fbid].name + "<span class='message_x' data-id='" + fbid + "'>x</span></span>&nbsp;";
            return html;
        },

        htmlRecipsList: function() {
            var recipHtmls = [];
            this.getRecipElts().each(function() {
                var outerHtml = $(this).clone().wrap('<p>').parent().html();
                recipHtmls.push(outerHtml);
            });
            return this.commafy(recipHtmls);
        },

        reformatRecipsList: function() {
            var recipsHtml = this.htmlRecipsList();
            $('#'+this.RECIPS_LIST_CONTAINER).empty().append(recipsHtml);
        },

        htmlRecipAdded: function(fbid) {
          var name = this.friendFromFbid[fbid].name;
          var html = "<div class='added_friend' id='added-"+ fbid + "'>" + name;
          html += "<div class='added_x' data-id='" + fbid + "'>x</div></div>";
          return html;
        },

        /* advances the active button (if current active is already ahead of index param, it does nothing) */
        activateButton: function(buttons, requestIdx) {
        ...
        activateSuggestButton: function() {
            return this.activateButton(buttons, 2);
        },

        activateShareButton: function() {
            return this.activateButton(buttons, 3);
        },

        commafy: function(things) {
        ...
        selectFriend: function() {
            ...
            for (index in arguments) {
                ...
                if (this.isRecip(fbid)) {
                    continue;
                }

                // If we've used the suggest button, we should have a recips list container,
                // so stick it in there and reformat
                if ($('#' + this.RECIPS_LIST_CONTAINER).length > 0) {
                    $('#' + this.RECIPS_LIST_CONTAINER).append(this.htmlRecip(fbid));
                    this.reformatRecipsList();
                } else { // otherwise, insert at cursor
                    this.insertRecipAtCursor(this.htmlRecip(fbid));
                }

                this.syncFriendBoxes();  // update the appearance of the friend box
                this.activateSuggestButton();  // advance the button highlight
                novelIds.push(fbid);
            }
            ...
            if (faces.debug) {
                ...
                events.record(selection_type, {friends: novelIds});
            }
        ...
        */
        unselectFriend: function(fbid) {
            if (this.isRecip(fbid)) {
                ...
                if (faces.debug) {
                    ...
                    events.record(unselection_type, {friends: [fbid]});
                }
                return true;
            } else {
                return false;
        ...
        syncFriendBoxes: function() {
            ...
            var recipIds = this.getRecipFbids();
            for (var i=0; i<recipIds.length; i++) {
                var fbid = recipIds[i];
                if (($('#added-'+fbid).length == 0) && ($('#friend-'+fbid).length == 0)) {
                      $('#picked_friends_container').append(this.htmlRecipAdded(fbid))
                }
            }
        },
        ...
        insertRecipAtCursor: function(html) {

            var sel, range;
            if ( this.elementContainsSelection($('#message_form_editable').get(0)) ) {
        ...
        useSuggested: function() {
            var msgs = this.suggested_messages;
            events.record('suggest_message_click');

            // If they don't have anyone checked, using the suggested message adds everyone
            if (this.getRecipFbids().length == 0) {
                this.selectAll(true);
            }
            ...
            var msgNamesContHtml = "<span id='" + this.RECIPS_LIST_CONTAINER + "'></span>";
            var recipsHtml = this.htmlRecipsList();  // these are going to get blown away, so capture them now
            $('#message_form_editable').empty().append(msgPre, msgNamesContHtml, msgPost);
            $('#'+this.RECIPS_LIST_CONTAINER).append(recipsHtml);
        ...
        /* selects all friends */
        selectAll: function(skipRecord) {
            if (!skipRecord) {
                events.record('select_all_click');
            }
            this.activateSuggestButton();
            ...
            var fbid,
                ...
                count = this.getRecipFbids().length;
            for (var i = 0; i < divs.length; i++) {
                if (count >= faces.max_face) {
        ...
        toggleFriend: function(fbid) {
            if (this.isRecip(fbid)) {
                  this.unselectFriend(fbid);
            }
            else {
                  this.selectFriend(fbid);
            }
        },

        /* called when someone suppresses a friend (X in faces list) */
        doReplace: function(old_fbid) {
            var div_id = '#wrapper-'+old_fbid;

            // Remove the friend from the messages
            this.unselectFriend(old_fbid);
            ...
            if (this.faceFriends.length > 0) {
                ...
                // templating...
                var fbid = this.faceFriends.shift();
                var fname = $("<div/>").html(this.friendFromFbid[fbid].fname).text();
                var lname = $("<div/>").html(this.friendFromFbid[fbid].lname).text();

                // Update the friends shown
                this.friendHTML(old_fbid, fbid, fname, lname, div_id);
                ...
                this.friendHTML(old_fbid, '', '', '', div_id);
                // $(div_id).remove();
            }
        ...
        friendHTML: function(oldid, id, fname, lname, div_id) {
            var new_html,
                self = this;
            $.ajax({
                type: "POST",
                ...
                dataType: 'html',
                data: {
                    userid: faces.user.fbid, // edgeflip.User constructed in frame_faces.html
                    ...
                    fname: fname,
                    lname: lname,
                    campaignid: faces.campaignid, // campaignid and contentid set in frame_faces.html
                    contentid: faces.contentid
                },
                ...
                success: function(data, textStatus, jqXHR) {
                    if (id) {
                        ...
                        $('#wrapper-' + id + ' .xout').click(
                            function(e) { e.stopImmediatePropagation(); self.doReplace( $(this).data('id') ); } );
                    } else {
        ...
        doShare: function(recursive) {
            ...
            if (!recursive)
                ...
                // share_click once.
                events.record('share_click');

            FB.login(function(response) {
                // FB.login will tell you if a user is authorized but will not tell 
                // you which permissions they have granted us.
                var perm_check = $.ajax({
                    url: 'https://graph.facebook.com/' + faces.user.fbid + '/permissions/',
                    data: {access_token: faces.user.token}
                });
                perm_check.done(function(resp) {
                    if (resp.data[0].publish_actions) {
                        events.record('publish_accepted'); 
                        this.sendShare();
                    } else {
                        events.record('publish_declined');
                        if (confirm("Without permission to publish, we're unable to share this message with your friends. \n\nClick OK to grant permission, or CANCEL to leave the page")) {
                            events.record('publish_reminder_accepted');
                            this.doShare(true);
                        } else {
                            events.record('publish_reminder_declined', {
                                complete: function() {
                                    faces.outgoingRedirect(faces.errorURL); // set in frame_faces.html
                                }
                            });
        ...
        recordShare: function(actionid, shareMsg, recips) {
            /* records share event on edgeflip servers; redirects user to thank you page */
            events.record('shared', {
                actionid: actionid,
                friends: recips,
                shareMsg: shareMsg,
                complete: function() {
                    faces.outgoingRedirect(faces.thanksURL); // set in frame_faces.html
                }
            });
        },

        helperTextDisappear: function() {
            $('#message_helper_txt').remove();
        },

        setDropdown: function(friends) {
        ...
        init: function() {
            ...
            $( function() {
                ...
                for (fbid in self.friendFromFbid) {
                    friend = self.friendFromFbid[fbid];
                    pickFriends.push({'value': friend.fbid, 'label': friend.name});
                }

                self.setDropdown(pickFriends);
            } );
        }
